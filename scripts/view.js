...
class PublicDomainView {
  ...
  populatePageData() {
    ...
    if (this.elements.thumbnail && this.thumbnail) {
      ...
      this.elements.thumbnail.alt = `Cover of ${this.title} by ${
    ...
    if (this.elements["publication-date"]) {
      this.elements["publication-date"].textContent =
        this.publicationDate || "Year Unknown";
    }

    if (this.elements["status-title"]) {
      this.elements["status-title"].textContent =
        this.publicDomainStatus.status || "Unknown Status";
      switch (this.publicDomainStatus.status) {
        ...
        case "Likely Copyrighted":
          this.elements["status-title"].classList.add(
            "status-likely-copyright"
          );
          break;
        case "Copyrighted":
    ...
    if (this.elements["status-badge"]) {
      this.elements["status-badge"].textContent =
        this.publicDomainStatus.status || "Unknown Status";
      switch (this.publicDomainStatus.status) {
        case "Public Domain":
          this.elements["status-badge"].classList.add("badge-green");
          break;
        case "Unknown":
          this.elements["status-badge"].classList.add("badge-gray");
          break;
        case "Likely Copyrighted":
          this.elements["status-badge"].textContent = "Copyrighted";
          this.elements["status-badge"].classList.add("badge-red");
          break;
        case "Copyrighted":
          this.elements["status-badge"].classList.add("badge-red");
          break;
    ...
    if (this.elements["status-description"]) {
      this.elements["status-description"].textContent =
        this.publicDomainStatus.description || "Status information unavailable";
    }
    ...
    if (this.elements["read-more-wiki"]) {
      console.log({ wiki: this.wikiUrl });
      this.elements["read-more-wiki"].setAttribute("href", this.wikiUrl);
    }
    ...
    if (this.elements["book-display"]) {
      ...
      this.elements["book-display"].setAttribute(
        "aria-label",
  ...
  async fetchBook(title) {
    ...
    try {
      ...
      if (data.type === "disambiguation") {
        return Promise.reject(new Error("Disambiguation page"));
      }
      ...
      const [publicationDate, author] = this.parsePublicationData(
        data.description || ""
      );
      ...
      return [publicationDate, author, description, thumbnail, wikiUrl];
    } catch (error) {
      clearTimeout(timeoutId);
